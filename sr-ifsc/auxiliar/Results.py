...
class Results:
        ...
        for res_dir in res_dir_pter:
            ...
            if self.loop["var"] is not None:
                ...
                if self.loop["var"] in self.beams['main'].index:
                    param = pd.read_csv(res_dir.path + "/parameters/beams/main.csv", header=0, index_col=0).squeeze().astype(object)
                    self._loop["values"].append(param[self.loop["var"]])
                #--
    ...
    def __cast_params_values(self):
        ...
        # Performance
        self._perform['max_time'] = float(self.perform['wait_time']) + float(self.perform['max_time'])
