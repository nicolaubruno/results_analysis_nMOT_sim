...
fig.suptitle("Ratio (" + r"$ \sigma_y / \sigma_z $" + ") as a function of \nthe laser detuning " + r"($\nabla B = " + del_B + " G/cm$)", size=16, y=0.95)
plt.close(1)
...
#--
# Simulated points
plt.plot(delta, std_dev_r[0] / std_dev_r[1], label="Simulation", marker='o', linestyle='--')

# Experiment
plt.plot(p_delta[0], p_std_dev[0] / p_std_dev[1], label="Experiment", marker='^', linestyle='--')

# Settings
plt.xlabel(r"$ \Delta\ [2\pi \times MHz] $")
plt.ylabel( r"$ \sigma_y / \sigma_z $")
plt.grid(linestyle="--")
