...
exp_off_Y = exp_off_Y / np.max(exp_on_Y) # N / N_{max}
#--

# Simulation EOM on
#--
sim_on_data = pd.read_csv("../../Dreon/Simulation/CSV Files/trap_depth_waist_off_EOM_on.csv").astype(float)
...
max_sim_Y = np.max(sim_on_Y)
sim_on_Y = sim_on_Y / max_sim_Y
#--

# Simulation EOM off
#--
sim_off_data = pd.read_csv("../../Dreon/Simulation/CSV Files/trap_depth_waist_off_EOM_off.csv").astype(float)
...
sim_off_Y = sim_off_Y / max_sim_Y # T / T_max
#--
#--
...
plt.suptitle("Cut off at 50/% of the trapped atoms and WITHOUT waist as a threshold", fontsize=16)
...
plt.ylabel(r"$ T_{capture} / T_{max} $")

# EOM Off
#--
# Max Point of the Simulation
max_X_idx = np.where(sim_off_Y == np.max(sim_off_Y))[0][0] 
plt.plot(sim_off_X, sim_off_Y, color=sns.color_palette()[3], linestyle="", marker="o", label=(("EOM Off (Cut off: " + r"$%.1f \Gamma$" + ")") % sim_off_X[max_X_idx]))
#--
...
plt.plot(sim_on_X, sim_on_Y, color=sns.color_palette()[0], linestyle="", marker="o", label=(("EOM On (Cut off: " + r"$%.1f \Gamma$" + ")") % sim_on_X[max_X_idx]))
#--

# Cut off laser detuning estimative
plt.fill_between([min_delta_cut, max_delta_cut], 0-max(sim_on_Y)*0.1, max(sim_on_Y)*1.1, color=sns.color_palette()[7], alpha=0.4, label="Cut off estimative")

# Set visualization
plt.ylim(0-max(sim_on_Y)*0.1, max(sim_on_Y)*1.1)
plt.xlim(min(sim_on_X)*1.1, max(sim_off_X)*0.8)
...
plt.plot(exp_off_X, exp_off_Y, color=sns.color_palette()[3], linestyle="", marker="s", label=(("EOM Off (Cut off: " + r"$%.1f \Gamma$" + ")") % exp_off_X[max_X_idx]))
#--
...
plt.plot(exp_on_X, exp_on_Y, color=sns.color_palette()[0], linestyle="", marker="s", label=(("EOM On (Cut off: " + r"$%.1f \Gamma$" + ")") % exp_on_X[max_X_idx]))
#--
